 ...
 */
public class DBFEngineTestNoMVStore {
    private static Connection connection;
    private static final String DB_NAME = "DBFEngineTest;MV_STORE=false";
    ...
    public void readDBFEncodingTest() throws SQLException {
        Statement st = connection.createStatement();
        st.execute("CALL FILE_TABLE("+StringUtils.quoteStringSQL(DBFEngineTestNoMVStore.class.getResource("encoding_test.dbf").getPath())+", 'DBFTABLE');");
        try ( // Query declared Table columns
                ResultSet rs = st.executeQuery("SELECT * FROM dbftable")) {
            assertTrue(rs.next());
    ...
    public void readDBFRussianWrongEncodingThenWriteThenRead() throws SQLException {
        ...
        st.execute("CALL DBFREAD("+StringUtils.quoteStringSQL(DBFEngineTestNoMVStore.class.getResource("sotchi.dbf").getPath())+", 'SOTCHI', 'cp1251');");
   ...
   // @Test
    public void readDBFNullDataTest() throws SQLException {
        Statement st = connection.createStatement();
        st.execute("drop table if exists dbftable");
        st.execute("CALL FILE_TABLE("+StringUtils.quoteStringSQL(DBFEngineTestNoMVStore.class.getResource("null_values.dbf").getPath())+", 'DBFTABLE');");
        try ( // Query declared Table columns
                ResultSet rs = st.executeQuery("SELECT * FROM dbftable")) {
            assertTrue(rs.next());
    ...
    public void readDBFCommaSeparatorNumericTest() throws SQLException {
        Statement st = connection.createStatement();
        st.execute("drop table if exists dbftable");
        st.execute("CALL FILE_TABLE("+StringUtils.quoteStringSQL(DBFEngineTestNoMVStore.class.getResource("comma_separator.dbf").getPath())+", 'DBFTABLE');");
        try ( // Query declared Table columns
                ResultSet rs = st.executeQuery("SELECT * FROM dbftable")) {
            assertTrue(rs.next());
