...
public class ST_MakeArcLine extends DeterministicScalarFunction {
    ...
    public ST_MakeArcLine() {
        addProperty(PROP_REMARKS, "Creates an elliptical arc, as a LineString centered at the given point with " +
    ...
    /**
     * Creates an elliptical arc, as a LineString  centered at the given point with the given start angle and
     * end angle size.
     *
     * @param p      Point
     ...
     * @return An arc line centered at the given point.
     */
    public static LineString execute(Point p, double distance, double startAngle, double angExtent) {
        if (p == null) {
            return null;
        }
        GSF.setCentre(new Coordinate(p.getX(), p.getY()));
        GSF.setSize(distance);
        LineString geom = GSF.createArc(startAngle, angExtent);
