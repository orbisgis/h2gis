...
public class FindGeometryMetadata extends DeterministicScalarFunction{
    ...
    /**
     * Extract the geometry metadata from it create table signature
     *
     * Examples:
     *
     * GEOMETRY GEOMETRY(POINT) GEOMETRY(POINT Z) GEOMETRY(POINT Z, 4326)
     * GEOMETRY(GEOMETRY, 4326)
     *
     * @param geometryTableSignature
     * @return an array of values with the following values order values[0] =
     * GEOMETRY_TYPE values[1] = COORD_DIMENSION values[2] = SRID values[3] =
     * TYPE
     * @throws SQLException
     */
