...
package org.h2gis.nosfs.functions.spatial.mesh;

import org.h2gis.sfs.unitTest.SpatialDBFactory;
import org.h2gis.nosfs.CreateSpatialExtension;
...
public class MeshFunctionTest {
    ...
    public static void tearUp() throws Exception {
        ...
        connection = SpatialDBFactory.createSpatialDataBase(MeshFunctionTest.class.getSimpleName(), false);
    ...
    public void test_ST_VORONOIPTS() throws Exception {
        st.execute("drop table if exists pts;\n" +
                ...
                "ST_ENVELOPE(ST_ACCUM(the_geom))) the_geom from PTS;");
        ResultSet rs = st.executeQuery("select ST_NUMGEOMETRIES(the_geom) cpt,st_perimeter(the_geom) lngth," +
                "st_npoints(the_geom) numpts  from voro;");
        assertTrue(rs.next());
